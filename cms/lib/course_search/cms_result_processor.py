...
class CmsSearchResultProcessor(SearchResultProcessor):
    """ SearchResultProcessor for CMS Search """
    _course_key = None
    _usage_key = None
    _module_store = None
    _course_blocks = {}
    ...
    def url(self):
        ...
        course_key = CourseKey.from_string(self._results_fields["course"])
        usage_key = UsageKey.from_string(self._results_fields["id"]).replace(course_key=course_key)
        return get_lms_link_for_item(usage_key)
