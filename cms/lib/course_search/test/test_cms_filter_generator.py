"""
Tests for the cms_filter_generator. This is copied from lms and may need to be changed.
"""
from mock import Mock, patch

from cms.lib.course_search.cms_filter_generator import CmsSearchFilterGenerator
...
class CmsSearchFilterGeneratorTestCase(ModuleStoreTestCase):
    ...
    def setUp(self):
        super(CmsSearchFilterGeneratorTestCase, self).setUp()
        self.build_courses()
    ...
    # This test is faild and leave it for now. The whole test instance may
    # need to be changed.
    ...
    #     """
    #     field_dictionary, filter_dictionary, _ = CmsSearchFilterGenerator.generate_field_filters(user=self.user)
    #
    ...
    def test_course_id_provided(self):
        ...
        """
        field_dictionary, filter_dictionary, _ = CmsSearchFilterGenerator.generate_field_filters(
            user=self.user,
    ...
    def test_excludes_site_org(self):
        ...
        """
        _, _, exclude_dictionary = CmsSearchFilterGenerator.generate_field_filters(user=self.user)
    ...
    def test_excludes_org_within(self):
        field_dictionary, _, exclude_dictionary = CmsSearchFilterGenerator.generate_field_filters(user=self.user)
        self.assertNotIn('org', exclude_dictionary)
