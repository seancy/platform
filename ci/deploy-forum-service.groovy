...
pipeline {
    ...
    stages{
        ...
        stage('Deploy forum service') {
            steps {
                dir('configuration/playbooks') {
                    sh """
                    . /tmp/.venv2/bin/activate
                    ansible-playbook -i ../../inventory/hosts.ini -l "${params.CLIENT}_tenant" -u ubuntu --private-key /opt/instanceskey/"${ec2_location}"_platform_key.pem --vault-password-file "${key_file}" lt_forum.yml
                    """
                }
            }
        }
