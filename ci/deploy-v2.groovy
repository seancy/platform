                        ...
                        timeout(time:2) {
                            ...
                            for (item in chooseOptions2) {
                                ...
                                if (item.key == 'CMS' && item.value == true) {
                                    def cms_folder_name = ''
                                    compile_theme = []
                                    cms_folder_name = 'eucalyptus-' + ec2_region.toLowerCase()
                ...
                dir('configuration/playbooks') {
                    script {
                        if (stage_auto_proceed == false) {
                            sh """
                            . /tmp/.venvec2/bin/activate
                            ansible-playbook -i "${instance_ip}" -u ubuntu --private-key /opt/instanceskey/"${ec2_location}"_platform_key.pem --vault-password-file "${key_file}" --tags "${tag_theme}" -e "{'LT_THEME': ${compile_theme}}" lt_pipeline_jobs.yml
                            """
                        } else if (stage_auto_proceed == true) {
                            sh """
                            . /tmp/.venvec2/bin/activate
